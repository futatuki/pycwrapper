...
def gen_CCharPtr(etypename, base):
...
CCharPtr = gen_CCharPtr('CCharPtr', CObjPtr)

def gen_CUCharPtr(etypename, base):
    def __new__(cls, vals=None, int nelms=0, int is_const=False, **m):
        cdef CObjPtr ref
        ref = base.__new__(cls, vals, nelms, is_const, **m)
        if is_const:
            ref._c_base_type = 'const unsigned char'
        else:
            ref._c_base_type = 'unsigned char'
        ref._c_esize = sizeof(unsigned char)
        ref._mddict = { 'p_' : 0 }
        ref._madict = { 's_' : ['p_'] }
        return ref
    def __init__(CObjPtr self, vals=None, int nelms=0, int is_const=False, **m):
...
CUCharPtr = gen_CUCharPtr('CUCharPtr', CObjPtr)
