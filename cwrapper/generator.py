...
footer_template = """    # function body
    ...
    nc = type(etypename, (base,), attrdict.copy())
    base._typedict['%%(ctype)s'] = nc
    return nc
"""
...
def FileHeader(fname, import_text, pyver=None):
    if pyver is None:
        pyver = sys.version_info[0]
    return """# %s (for Python %s), generated by
#   $yfId$
%s""" % (fname, '>= 3' if pyver >= 3 else '2', import_text)
...
def GenStaticClsSrcFile(
    ...
    # write header
    pxdfile.write(FileHeader(fname + '.pxd', import_text, pyver))
    pyxfile.write(FileHeader(fname + '.pyx', import_text, pyver))
